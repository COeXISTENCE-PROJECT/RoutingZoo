...
class Simulator:
    ...
    def run_simulation_iteration(self, joint_action): #RK: Why iteration? Let's think about naming it: episode? It will be confusing later.
        ...
        #traci.vehicle.add(vehID='test',routeID="test",depart="0.00")

        # Simulation loop
        while should_continue:
            timestep = int(traci.simulation.getTime()) # RK: do we need this? or maybe add simulationStep everytime>
            #traci.vehicle.setRoute(vehID='test',edgeList=["top3D3", "D3D2", "D2C2", "C2C3", "C3D3", "D3top3"])
            #added_route = np.random.randint(low=1,high=9)
            #id_added = len(joint_action) + timestep
            #if timestep % 10 == 0:
            #    traci.vehicle.add(vehID=f'{id_added}',routeID=f"{added_route}") #random lista a routenak es egy random nev generator a vehiclenek
